...
const VagasList: React.FC = () => {
  ...
  const vagas: Vaga[] = [
    { id: 'v1', title: 'Desenvolvedor Frontend Sênior', company: 'Tech Solutions', location: 'São Paulo, SP', status: 'hiring', applicants: 15 },
    { id: 'v2', title: 'Analista de Marketing Digital', company: 'Creative Minds', location: 'Rio de Janeiro, RJ', status: 'open', applicants: 8 },
    { id: 'v3', title: 'Gerente de Projetos', company: 'Global Corp', location: 'Remoto', status: 'closed', applicants: 20 },
    { id: 'v4', title: 'UX/UI Designer Júnior', company: 'Design Studio', location: 'Belo Horizonte, MG', status: 'open', applicants: 12 },
  ];
  ...
  return (
    <div className={styles.vagasSection}>
            ...
            <tr key={vaga.id}>
              ...
              <td>
                <span className={`${styles.statusPill} ${styles[vaga.status]}`}>
                  {vaga.status === 'open' ? 'Aberta' : vaga.status === 'closed' ? 'Fechada' : 'Contratando'}
                </span>
              </td>
      ...
      {filteredVagas.length === 0 && (
        <p style={{ textAlign: 'center', marginTop: '20px', color: '#6b7280' }}>Nenhuma vaga encontrada.</p>
      )}
    </div>
  );
};
