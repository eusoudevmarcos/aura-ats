...
const columns: TableColumn<VagaWithRelations>[] = [
  ...
  {
    label: "Habilidades Chave",
    key: "habilidades",
    render: (_, row) =>
      row.habilidades && row.habilidades.length > 0
        ? row.habilidades.map((vh) => vh.habilidade.nome).join(", ")
        : "N/A",
  },
];
...
const VagaList: React.FC = () => {
  ...
  const filteredVagas = vagas.filter(
    (vaga) =>
      ...
      vaga.cliente?.nome?.toLowerCase().includes(searchQuery.toLowerCase()) ||
