import { AreaCandidato, TipoSocio } from "@prisma/client";
...
export function convertDataStoneToCandidatoDTO(
  ...
  const localizacoes =
    dataStoneData.addresses
      ...
      .map((addr) => ({
        ...
        uf: addr.district || "DF", // Default para DF se não informado
  ...
  // Constrói o DTO
  const dto: DataStoneCandidatoDTO = {
    areaCandidato: AreaCandidato.OUTRO, // Default, pode ser ajustado conforme necessário
    emails,
    contatos,
    ...
    pessoa: {
      ...
      dataNascimento: parseDate(dataStoneData.birthday),
      localizacoes,
